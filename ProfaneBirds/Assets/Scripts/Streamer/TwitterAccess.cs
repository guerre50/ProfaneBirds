...
namespace Streamer
{
	public class TwitterAccess
	{
		...
		private const string streamSampleURL = "https://stream.twitter.com/1.1/statuses/sample.json";
		...
		public delegate void AuthErrorHandler(string error);
		public AuthErrorHandler authErrorHandler { get; set;}
		...
		{		
			Debug.Log( "Starting connection to " + connectingRequest.RequestUri );
			// try to get stream
			Debug.Log (connectingRequest.RequestUri+(string)postString);
			HttpWebResponse newResponse = GetNewConnection ( (string)postString );
			
			Debug.Log( "Have response. Starting stream" );
			...
			while ( response == null )
			{
				...
				catch ( WebException e )
				{
					...
					if ( e.Status == WebExceptionStatus.ProtocolError )	// http level error
					{
						...
						if (authErrorHandler != null) authErrorHandler(errorResponse.StatusDescription);
					...
					else	// network level error
					{
						...
						if (authErrorHandler != null) authErrorHandler(e.Message);
